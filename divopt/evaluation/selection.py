...
from divopt.evaluation.simgraph import (
...
def maxmin(
    ...
    """
    fps = [morgan_from_smiles(s, radius, nbits) for s in smiles]
    ...
    picks = mmp.LazyBitVectorPickWithThreshold(fps, len(fps), n_picks, distance_threshold)
    return list(picks[0])
...
def taylor_butina(
    ...
    Returns:
        List[int]: Indices of selected compounds
    """
    n_neighbours = compute_number_neighbours(smiles, distance_threshold=distance_threshold, radius=radius, nbits=nbits)
