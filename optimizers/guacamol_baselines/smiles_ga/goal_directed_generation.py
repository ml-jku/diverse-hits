...
# remove scoring function argument and delay scoring.
def mutate(p_gene):
...
class ChemGEGenerator(GoalDirectedGenerator):
    ...
    def generate_optimized_molecules(
        ...
        if starting_population is None:
            ...
            if self.random_start:
                ...
                starting_population = self.top_k(
                    all_smiles, scoring_function, init_size
                )
            print("selecting initial population... Done")
        ...
        for generation in range(self.generations):
            ...
            smiles_gene_list = self.pool(joblist)
            smiles_list = [t[0] for t in smiles_gene_list]
            gene_list = [t[1] for t in smiles_gene_list]
            scores = scoring_function.score_list(smiles_list)
            new_population = [
                Molecule(c_score, c_smiles, c_gene)
                for (c_score, c_smiles, c_gene) in zip(scores, smiles_list, gene_list)
            ]

            # if scoring function changes due to memory rescore the old population
            if self.dynamic_score:
                old_population_smiles = [x.smiles for x in population]
                old_population_scores = scoring_function.score_list(
                    old_population_smiles
                )
                population = [
                    Molecule(s, x.smiles, x.genes)
                    for x, s in zip(population, old_population_scores)
                ]
