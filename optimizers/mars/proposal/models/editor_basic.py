        ...
        with torch.no_grad():
            ...
            for i, g in enumerate(dgl.unbatch(g)):
                ...
                if edits['act'][i].item() == 0: # del
                    ...
                    mask_del[off_edge:off_edge+n_edge] = 1.
                    ...
                    mask_add[off_node:off_node+n_node] = 1.
                    mask_arm[off_node+add_idx] = 1.
                ...
                off_node += n_node
                off_edge += n_edge
        prob_del = 1. * sum(prob_del) / (len(prob_del) + 1e-6)
        prob_add = 1. * sum(prob_add) / (len(prob_add) + 1e-6)
        prob_arm = 1. * sum(prob_arm) / (len(prob_arm) + 1e-6)
        
        ### losses
        loss_del = F.cross_entropy(pred_del, targ_del, reduction='none') # (tot_n_edge,)
        loss_add = F.cross_entropy(pred_add, targ_add, reduction='none') # (tot_n_node,)
        loss_arm = F.cross_entropy(pred_arm, targ_arm, reduction='none') # (tot_n_node,)
