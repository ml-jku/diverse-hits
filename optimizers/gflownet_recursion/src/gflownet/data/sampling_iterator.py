...
class SamplingIterator(IterableDataset):
    ...
    def __init__(
        ...
        self.device = device
        self.stream = stream
        self.sample_online_once = (
            True  # TODO: deprecate this, disallow len(data) == 0 entirely
        )
    ...
    def _idx_iterator(self):
        ...
        if self.stream:
            ...
            if worker_info is None:  # no multi-processing
                ...
                wid = worker_info.id
                start, end = int(np.round(n / nw * wid)), int(
                    np.round(n / nw * (wid + 1))
                )
    ...
    def __iter__(self):
        ...
        if self.log_dir is not None:
            ...
            self.generation_history_path = (
                f"{self.log_dir}/generated_mols_{self._wid}.csv"
            )
        ...
        for idcs in self._idx_iterator():
            ...
            if self.sample_cond_info:
                ...
                mols, flat_rewards = (
                    map(list, zip(*[self.data[i] for i in idcs]))
                    if len(idcs)
                    else ([], [])
                )
                flat_rewards = (
                    list(self.task.flat_reward_transform(torch.stack(flat_rewards)))
                    if len(flat_rewards)
                    else []
                )
            ...
            if num_online > 0:
                ...
                if self.algo.bootstrap_own_reward:
                    ...
                    online_flat_rew, m_is_valid = self.task.compute_flat_rewards(
                        mols, self.generation_history_path
                    )
                ...
                for hook in self.log_hooks:
                    extra_info.update(
                        hook(
                            ...
                            {
                                k: v[num_offline:]
                                for k, v in deepcopy(cond_info).items()
                            },
                        )
                    )
            ...
            if self.hindsight_ratio > 0.0:
                ...
                hindsight_idxs = (
                    torch.randperm(num_online)[: int(num_online * self.hindsight_ratio)]
                    + num_offline
                )
    ...
    def validate_batch(self, batch, trajs):
        for actions, atypes in [(batch.actions, self.ctx.action_type_order)] + (
            ...
            if hasattr(batch, "bck_actions")
            and hasattr(self.ctx, "bck_action_type_order")
            else []
        ):
            ...
            masked_action_is_used = 1 - mask_cat.log_prob(
                actions, logprobs=mask_cat.logits
            )
            num_trajs = len(trajs)
            batch_idx = torch.arange(
                num_trajs, device=batch.x.device
            ).repeat_interleave(batch.traj_lens)
            ...
            if masked_action_is_used.sum() != 0:
                ...
                raise ValueError(
                    "Found an action that was masked out",
                    trajs[traj_idx]["traj"][timestep],
                )
    ...
    def log_generated(self, trajs, rewards, flat_rewards, cond_info):
        ...
        focus_dir = (
            cond_info.get("focus_dir", torch.zeros((len(mols), 0)))
            .data.numpy()
            .tolist()
        )
        logged_keys = [
...
class SQLiteLog:
    ...
    def insert_many(self, rows, column_names):
        assert all(
            [type(x) is str or not isinstance(x, Iterable) for x in rows[0]]
