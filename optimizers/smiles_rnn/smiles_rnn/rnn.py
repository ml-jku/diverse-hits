...
class Model:
    ...
    def sample_native(
        self, num=128, batch_size=128, temperature=1.0
    ) -> Tuple[List, np.array]:
        """
    ...
    def sample_sequences_and_smiles(
        self, batch_size=128, temperature=1.0
    ) -> Tuple[
        torch.Tensor,
        List,
        torch.Tensor,
    ...
    def _sample(
        ...
        action_probs = [
            torch.zeros([batch_size, 1], dtype=torch.float, requires_grad=True)
        ]
        action_log_probs = [
            torch.zeros([batch_size, 1], dtype=torch.float, requires_grad=True)
        ]
        ...
        for _ in range(self.max_sequence_length - 1):
            ...
            action_probs.append(
                torch.tensor([p[a] for p, a in zip(probabilities, input_vector)]).view(
                    -1, 1
                )
            )
            action_log_probs.append(
                torch.tensor([p[a] for p, a in zip(log_probs, input_vector)]).view(
                    -1, 1
                )
            )
            if self.network._get_name() == "RNNCritic":
