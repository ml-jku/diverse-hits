...
import torch
from .rnn import Model as RNNModel
from .transformer import Model as TransformerModel
from .GTr import Model as StableTransformerModel
from . import utils
...
class ReinforcementLearning:
    ...
    def _score(self, smiles, step):
        try:
            scores = torch.tensor(self.scoring_function(smiles))
...
class BestAgentReminder(ReinforcementLearning):
    ...
    def _sample_best_batch(self, batch_size):
        ...
        with torch.no_grad():
            (
                ...
                best_log_probs,
                _,
            ) = self.best_agent.sample_sequences_and_smiles(batch_size)
    ...
    def _train_step(self, step):
        ...
        (
            ...
            best_log_probs,
            _,
        ) = self._sample_best_batch(
            self.batch_size
        )  # samples half the batch size samples

        # Score
        all_scores = self._score(smiles + best_smiles, step)
        scores = utils.to_tensor(all_scores[: len(smiles)]).to(self.device)
        best_scores = utils.to_tensor(all_scores[len(smiles) :]).to(self.device)
