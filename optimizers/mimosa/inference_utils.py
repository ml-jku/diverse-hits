...
def optimize_single_molecule_one_iterate(smiles, gnn):
    ...
    (
        ...
        origin_adjacency_matrix,
        leaf_extend_idx_pair,
    ) = smiles2graph(smiles)
    try:
    ...
    for node_mat, adj_mat, mask_idx in feature_lst:
        ...
        for substru_idx, word in zip(top_idxs, top_words):
            ...
            if (
                type(leaf_atom_idx_lst) == int
            ):  ### int: single atom;   else: list of integer
            ...
            for leaf_atom_idx in leaf_atom_idx_lst:
                for new_bond in bondtype_list:
                    if ith_substructure_is_atom(substru_idx):
                        new_smiles = add_atom_at_position(
                            editmol=origin_mol,
                            position_idx=leaf_atom_idx,
                            new_atom=word,
                            new_bond=new_bond,
                        )
                        new_smiles_set.add(new_smiles)
                    else:
                        new_smiles_batch = add_fragment_at_position(
                            editmol=origin_mol,
                            position_idx=leaf_atom_idx,
                            fragment=word,
                            new_bond=new_bond,
    ...
    new_smiles_set = set(
        [new_smiles for new_smiles in new_smiles_set if new_smiles != None]
    )
    return new_smiles_set


def optimize_single_molecule_all_generations(
    input_smiles, gnn, oracle, generations, population_size, lamb
):
    smiles2f = dict()
    traceback_dict = dict()
...
def inference_single_molecule(
    input_smiles, gnn, result_file, generations, population_size
):
    (
        best_mol_score_list,
        input_score,
        ...
        input_smiles, gnn, oracle, generations, population_size
    )
    return calculate_results(
        input_smiles, input_score, result_file, best_mol_score_list, oracle
    )


def inference_molecule_set(
    input_smiles_lst, gnn, result_file, generations, population_size
):
    score_lst = []
    for input_smiles in input_smiles_lst:
        if not is_valid(input_smiles):
            continue
        result = inference_single_molecule(
            input_smiles, gnn, result_file, generations, population_size
        )
        if result is None:
